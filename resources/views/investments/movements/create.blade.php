<x-app-layout>
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Nuevo movimiento</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Cuenta: <span class="font-semibold">{{ $investment->name }}</span></p>
            </div>
            <a href="{{ route('investments.movements.index', $investment) }}" class="text-sm font-medium hover:underline">Volver</a>
        </div>

        <div class="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm">
            <form method="POST" action="{{ route('investments.movements.store', $investment) }}" class="p-6 space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <x-form.select
                        ...
                        :selected="old('currency', $investment->default_currency)"
                        required
                    />
