<x-app-layout>
    ...
    <div class="space-y-4">
        ...
        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-sm p-4">
            <form method="GET" action="{{ route('instruments.index') }}" class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">

                <div class="md:col-span-5">
                    <label class="block text-sm mb-1 text-gray-600 dark:text-gray-300">Buscar</label>
                    <input name="q" value="{{ $q }}" placeholder="Ej: KO, AAPL, GGAL..."
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="type" class="w-full rounded-xl border-gray-200 dark:border-gray-700 dark:bg-gray-900">
                        ...
                        <option value="cedear" {{ $type==='cedear'?'selected':'' }}>CEDEAR</option>
                        <option value="accion" {{ $type==='accion'?'selected':'' }}>Acción</option>
                        <option value="fci" {{ $type==='fci'?'selected':'' }}>FCI</option>
                        <option value="crypto" {{ $type==='crypto'?'selected':'' }}>Crypto</option>
                    </select>
                </div>

                <div class="md:col-span-3">
                    <label class="block text-sm mb-1 text-gray-600 dark:text-gray-300">Watchlist</label>
                    <select name="only" class="w-full rounded-xl border-gray-200 dark:border-gray-700 dark:bg-gray-900">
                        <option value="">Todos</option>
                        <option value="watchlist" {{ $only==='watchlist'?'selected':'' }}>Solo en mi watchlist</option>
                        <option value="not" {{ $only==='not'?'selected':'' }}>Solo NO en mi watchlist</option>
                    </select>
                </div>
        ...
        <form method="POST" action="{{ route('watchlist.bulk') }}" id="bulkForm" class="space-y-3">
            @csrf
            ...
            <div class="flex flex-col sm:flex-row gap-2 sm:items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <select name="horizon" class="rounded-xl border-gray-200 dark:border-gray-700 dark:bg-gray-900 text-sm">
                    ...
                    <span class="text-xs text-gray-500 dark:text-gray-400">
                        Tip: podés filtrar y después seleccionar.
                    </span>
                </div>
                ...
                <div class="flex gap-2">
                    ...
                    <button type="submit" name="action" value="remove"
                            ...
                            onclick="return confirm('¿Quitar los seleccionados de tu watchlist?')">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                        <thead class="bg-gray-50 dark:bg-gray-800/50 text-gray-600 dark:text-gray-300">
                            <tr>
                                <th class="text-left font-medium px-4 sm:px-6 py-3 w-12"></th>
                                ...
                                <th class="text-left font-medium px-4 sm:px-6 py-3">En watchlist</th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            @forelse($instruments as $inst)
                                @php
                                    $inWatch = isset($watchMap[$inst->id]);
                                @endphp
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40">
                                    <td class="px-4 sm:px-6 py-3">
                                        <input type="checkbox" name="instrument_ids[]"
                                               value="{{ $inst->id }}" class="rowCheck rounded">
                                    </td>
                                    ...
                                    <td class="px-4 sm:px-6 py-3">
                                        @if($inWatch)
                                            <span class="px-2 py-1 rounded-full text-xs bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-200">
                                                Sí
                                            </span>
                                        @else
                                            <span class="px-2 py-1 rounded-full text-xs bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-200">
                                                No
                                            </span>
                                        @endif
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="px-4 sm:px-6 py-8 text-center text-gray-500 dark:text-gray-400">
