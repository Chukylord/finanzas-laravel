<x-app-layout>
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Nuevo egreso</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Registrá un gasto manual.</p>
            </div>
        ...
        <div class="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm">
            <form method="POST" action="{{ route('expenses.store') }}" class="p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <x-form.input label="Fecha" name="date" type="date" required />
                    <x-form.input label="Monto" name="amount" type="number" step="0.01" placeholder="Ej: 15000" required />
                </div>

                <x-form.input
                    label="Detalle (opcional)"
                    name="description"
                    placeholder="Ej: Supermercado / Combustible / Netflix"
                />
