    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400">Actualizá categoría, subcategoría, fecha o monto.</p>
            </div>
            ...
            <form method="POST" action="{{ route('incomes.update', $income) }}" class="p-6 space-y-5">
                    ...
                    <select name="subcategory_id"
                        ...
                        @foreach($subcategories as $sub)
                            <option value="{{ $sub->id }}"
                                    ...
                                    {{ (string)old('subcategory_id', $income->subcategory_id) === (string)$sub->id ? 'selected' : '' }}>
                                {{ $sub->category?->name }} · {{ $sub->name }}
                            </option>
                        @endforeach
                    </select>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        Sirve para clasificar ingresos ya cargados con más detalle.
                    </p>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <x-form.input
                        ...
                        name="date"
                        type="date"
                        :value="old('date', \Carbon\Carbon::parse($income->date)->toDateString())"
                        required
                    />
